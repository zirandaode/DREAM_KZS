function Z = dream_kzs(N,T,Nx,Ny,Obs,sd,range,p_k,t1,t2,Ne)
% Please modify prior.m, pdf.m, error_model.m for your own problem

[delta,c,c_star,n_CR,p_g] = deal(1,0.05,1e-12,3,0.2);                      % Default of algorithmic variables
k = 10; p_s = 0.1; m0 = max(N,20*Nx); n_d = 3*delta; id = nan(N,1);        % Algorithmic variables
[J,n_id] = deal(zeros(1,n_CR));                                            % Variables selection probability crossover
CR = [1:n_CR]/n_CR; p_CR = ones(1,n_CR)/n_CR;                              %#ok<NBRAK> % Crossover value and selection probability
Z = prior(m0,Nx,range); X = Z(m0-N+1:m0,1:Nx);                             % Create initial archive Z and initial population X
[P,Y] = pdf(X,Obs,sd,range); m = m0;                                       % Compute log-density and model responses of initial population
f = fopen('x.bin','w+','n'); fwrite(f,X','double'); fclose(f);             % Store model parameters of initial population
f = fopen('y.bin','w+','n'); fwrite(f,Y','double'); fclose(f);             % Store model responses of initial population
f = fopen('p.bin','w+','n'); fwrite(f,P','double'); fclose(f);             % Store log-densities of initial population
