...
Nx = 121;          % Dimension of model parameters
...
range = repmat([-5 5],Nx-1,1);      
range = [range; [0 0.02]];            % Range of the model and error model parameters
...
Obs = yreal + 0.01*ones(Ny,1).*randn(size(yreal)); % The measurements perturbed with white noise
